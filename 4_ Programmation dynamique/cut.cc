...
//    cut_indices = new vector<int>;
    vector<double> memo(L+1,0.);
    double best = 0.;
    for (int i=1; i<=L; i++) {
        for (PricedCut p : cuts) {
            if (i-p.length>=0) {
                best = max(best,memo[i-p.length]+p.price);
                memo[i]=best;
            }
        }
    }
    ...
    for (int i=L; i>=1; i--) {
        if (memo[i]!=k) {
            for (int j=0; j<cuts.size(); j++) {
                if (memo[i+1-cuts[j].length] == k-cuts[j].price) {
