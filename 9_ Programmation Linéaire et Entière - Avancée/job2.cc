...
for (int i = 0; i < local_dep.size(); ++i){
  for (int j = 0; j < num_machines; ++j){
    MPConstraint* ct =  solver.MakeRowConstraint(-2, 0);      
    ct->SetCoefficient(var[local_dep[i].first][j],1);
    ct->SetCoefficient(var[local_dep[i].second][j],-1);
  }
}
