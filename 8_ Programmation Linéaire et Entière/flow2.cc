...
vector<FlowOnArc> BestFlow2(int num_nodes, const vector<BackboneArc>& backbone,
  ...
  vector<MPVariable*> utilisation_vars;
  for (int i = 0; i < num_arcs; ++i){
    utilisation_vars.push_back(solver.MakeNumVar(0, 1-max_free_utilization_ratio,"Flow(demand:" + std::to_string(i) +")"));
  }

  // Contraintes de capacité.
  for (int a = 0; a < num_arcs; ++a) {
    MPConstraint* ct = solver.MakeRowConstraint(0, backbone[a].capacity*max_free_utilization_ratio, "ArcCapacity" + std::to_string(a));
    for (int d = 0; d < num_demands; ++d) {
      ct->SetCoefficient(flow_var[d][a], 1.0);
    }
    ct->SetCoefficient(utilisation_vars[a], -backbone[a].capacity);
  }
  ...
  for (int i = 0; i < num_arcs; ++i){
      objective->SetCoefficient(utilisation_vars[i], -penalty_cost);
  }
